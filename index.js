Date.prototype.format = function (format) {
    ...
    formattedDate = formattedDate.replace('Y', this.getFullYear())
        ...
        .replace('d', this.getDate().toString().length < 2 ? '0' + this.getDate().toString() : this.getDate());

    // TimeFormatting
    formattedDate = formattedDate.replace('H', this.getHours())
        .replace('i', this.getMinutes())
        .replace('s', this.getSeconds().toString().length < 2 ? '0' + this.getSeconds().toString() : this.getSeconds());
...
exports.ActiveRecord = require('./lib/ActiveRecord/index');
